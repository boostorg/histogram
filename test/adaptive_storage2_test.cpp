...
#include <boost/histogram/storage/adaptive_storage2.hpp>
...
template <typename T> adaptive_storage2<> prepare(unsigned n = 1) {
  adaptive_storage2<> s(n);
  s.increase(0);
...
template <> adaptive_storage2<> prepare<void>(unsigned n) {
  adaptive_storage2<> s(n);
  return s;
}

template <> adaptive_storage2<> prepare<detail::weight>(unsigned n) {
  adaptive_storage2<> s(n);
  s.increase(0, 1.0);
  return s;
}

template <> adaptive_storage2<> prepare<detail::mp_int>(unsigned n) {
  adaptive_storage2<> s(n);
  s.increase(0);
...
struct storage_access {
  template <typename T> static adaptive_storage2<> set_value(unsigned n, T x) {
    adaptive_storage2<> s = prepare<T>(n);
    using Array = adaptive_storage2<>::array<T>;
    get<Array>(s.buffer_)[0] = x;
    return s;
  }
};
...
template <> void equal_impl<void>() {
  adaptive_storage2<> a(1);
  adaptive_storage2<> b(1);
  BOOST_TEST_EQ(a.value(0), 0.0);
  BOOST_TEST_EQ(a.variance(0), 0.0);
  BOOST_TEST(a == b);
  b.increase(0);
  BOOST_TEST(!(a == b));
  adaptive_storage2<> c(1);
  BOOST_TEST(c == a);
}

template <typename T> void increase_and_grow_impl() {
  auto tmax = std::numeric_limits<T>::max();
  adaptive_storage2<> s = storage_access::set_value<T>(2, tmax - 1);
  auto n = s;
  auto n2 = s;
...
int main() {
  ...
  // equal_operator
  {
    ...
    // // special case
    // adaptive_storage2<> a = storage_access::set_value(1, unsigned(0));
    // adaptive_storage2<> b(1);
  }
