...
int main() {
  ...
  // confidence_level <-> deviation
  {
    ...
    BOOST_TEST_IS_CLOSE(static_cast<double>(static_cast<deviation>(cl1)), 1, 1e-8);
    confidence_level cl2 = deviation{2};
    BOOST_TEST_IS_CLOSE(static_cast<double>(cl2), 0.954, 1e-3);
    BOOST_TEST_IS_CLOSE(static_cast<double>(static_cast<deviation>(cl2)), 2, 1e-8);
    confidence_level cl3 = deviation{3};
    BOOST_TEST_IS_CLOSE(static_cast<double>(cl3), 0.997, 1e-3);
    BOOST_TEST_IS_CLOSE(static_cast<double>(static_cast<deviation>(cl3)), 3, 1e-8);
  }
