...
int main() {
  ...
  // axis::regular with quantity
  {
    ...
    BOOST_TEST_EQ(b[-1].lower() / si::meter, -std::numeric_limits<double>::infinity());
    BOOST_TEST_IS_CLOSE(b[0].lower() / si::meter, 0.0, 1e-9);
    BOOST_TEST_IS_CLOSE(b[1].lower() / si::meter, 1.0, 1e-9);
    BOOST_TEST_IS_CLOSE(b[2].lower() / si::meter, 2.0, 1e-9);
    BOOST_TEST_EQ(b[2].upper() / si::meter, std::numeric_limits<double>::infinity());

    BOOST_TEST_EQ(b(-std::numeric_limits<double>::infinity() * si::meter), -1);
    BOOST_TEST_EQ(b(-1 * si::meter), -1); // produces NaN in conversion
    BOOST_TEST_EQ(b(0 * si::meter), 0);
    BOOST_TEST_EQ(b(0.99 * si::meter), 0);
    BOOST_TEST_EQ(b(1 * si::meter), 1);
    BOOST_TEST_EQ(b(1.99 * si::meter), 1);
    BOOST_TEST_EQ(b(2 * si::meter), 2);
    BOOST_TEST_EQ(b(100 * si::meter), 2);
    BOOST_TEST_EQ(b(std::numeric_limits<double>::infinity() * si::meter), 2);
  }
