...
mkdir -p ${BOOST_DIR}
BOOTSTRAP_PATCH_REGEX="s|\( *using python.*\);|\1: $(python build/get_python_include.py) ;|"
echo "Bootstrap patch line: ${BOOTSTRAP_PATCH_REGEX}"
if [[ -z "$(ls -A ${BOOST_DIR})" ]]; then
  ...
  (cd ${BOOST_DIR} && ./bootstrap.sh > /dev/null && \
   ...
   sed -i "${BOOTSTRAP_PATCH_REGEX}" project-config.jam && \
   cat project-config.jam && \
