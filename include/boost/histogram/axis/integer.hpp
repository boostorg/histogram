...
class integer : public base<MetaData, Options>,
                ...
                public optional_integer_mixin<integer<IntType, MetaData, Options>,
                                              IntType, test(Options, option::growth)> {
...
class optional_integer_mixin<Derived, V, true> {
  using value_type = V;
...
private:
  auto update_impl(std::false_type, int x) noexcept {
    const auto i = x - static_cast<Derived*>(this)->min_;
    if (i >= 0) {
      if (i < static_cast<Derived*>(this)->size()) return std::make_pair(i, 0);
      const auto n = i - static_cast<Derived*>(this)->size() + 1;
      static_cast<Derived*>(this)->grow(n);
      return std::make_pair(i, -n);
    }
    static_cast<Derived*>(this)->min_ += i;
    static_cast<Derived*>(this)->grow(-i);
