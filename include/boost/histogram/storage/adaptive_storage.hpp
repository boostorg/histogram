...
using type_to_int = mpl::map<
    mpl::pair<int8_t, mpl::int_<1>>, mpl::pair<int16_t, mpl::int_<2>>,
    mpl::pair<int32_t, mpl::int_<3>>, mpl::pair<int64_t, mpl::int_<4>>,

    mpl::pair<uint8_t, mpl::int_<1>>, mpl::pair<uint16_t, mpl::int_<2>>,
    mpl::pair<uint32_t, mpl::int_<3>>, mpl::pair<uint64_t, mpl::int_<4>>,

    mpl::pair<mp_int, mpl::int_<5>>, mpl::pair<weight, mpl::int_<6>>>;

using int_to_type = mpl::map<
    mpl::pair<mpl::int_<1>, uint8_t>, mpl::pair<mpl::int_<2>, uint16_t>,
    mpl::pair<mpl::int_<3>, uint32_t>, mpl::pair<mpl::int_<4>, uint64_t>,
    mpl::pair<mpl::int_<5>, mp_int>, mpl::pair<mpl::int_<6>, weight>>;

template <typename T> using to_int = typename mpl::at<type_to_int, T>::type;
...
using next_storage_type =
    typename mpl::at<int_to_type, typename mpl::next<to_int<T>>::type>::type;
...
template <typename Buffer, typename O> struct add_impl<Buffer, mp_int, O> {
  static void apply(Buffer &b, const std::size_t i, const O &o) {
    b.template at<mp_int>(i) += o;
  }
};

template <typename Buffer, typename O> struct add_impl<Buffer, weight, O> {
  static void apply(Buffer &b, const std::size_t i, const O &o) {
    b.template at<weight>(i) += o;
  }
};

template <typename Buffer, typename OtherBuffer, typename T> struct cmp_impl {
...
template <template <class> class Allocator> struct buffer {
  explicit buffer(std::size_t s = 0) : size_(s), ptr_(nullptr) {}
  ...
  buffer(const buffer &o) : size_(o.size_) {
    ...
    case 1:
      create<uint8_t>();
      copy_from<uint8_t>(o.ptr_);
      break;
    case 2:
      create<uint16_t>();
      copy_from<uint16_t>(o.ptr_);
      break;
    case 3:
      create<uint32_t>();
      copy_from<uint32_t>(o.ptr_);
      break;
    case 4:
      create<uint64_t>();
      copy_from<uint64_t>(o.ptr_);
      break;
    case 5:
      create<mp_int>();
      copy_from<mp_int>(o.ptr_);
      break;
    case 6:
      create<weight>();
      copy_from<weight>(o.ptr_);
      break;
    }
  }
  ...
  buffer(buffer &&o) : type_(o.type_), size_(o.size_), ptr_(o.ptr_) {
    o.size_ = 0;
    o.type_ = type();
    o.ptr_ = nullptr;
  }
  ...
  template <typename T> void copy_from(const void *p) {
    std::copy(static_cast<const T *>(p), static_cast<const T *>(p) + size_,
              static_cast<T *>(ptr_));
  }

  template <typename T, typename U = next_storage_type<T>>
  typename std::enable_if<(to_int<T>::value < to_int<U>::value)>::type grow() {
    Allocator<U> a;
  ...
  template <typename T> inline T &at(std::size_t i) {
    return static_cast<T *>(ptr_)[i];
  }

  template <typename T> inline const T &at(std::size_t i) const {
    return static_cast<const T *>(ptr_)[i];
  }
...
template <template <class> class Allocator>
template <typename OtherStorage, typename>
adaptive_storage<Allocator> &adaptive_storage<Allocator>::
operator=(const OtherStorage &o) {
  using T = typename OtherStorage::value_type;
  buffer_.destroy_any();
...
template <template <class> class Allocator>
inline void adaptive_storage<Allocator>::increase(std::size_t i) {
  switch (buffer_.type_.id_) {
  case 0:
    buffer_.template initialize<uint8_t>(); // and fall through
  case 1:
    buffer_.template increase<uint8_t>(i);
    break;
  case 2:
    buffer_.template increase<uint16_t>(i);
    break;
  case 3:
    buffer_.template increase<uint32_t>(i);
    break;
  case 4:
    buffer_.template increase<uint64_t>(i);
    break;
  case 5:
    ++(buffer_.template at<detail::mp_int>(i));
    break;
  case 6:
    ++(buffer_.template at<detail::weight>(i));
    break;
  }
}

template <template <class> class Allocator>
inline void adaptive_storage<Allocator>::increase(std::size_t i, value_type w) {
  buffer_.template convert<detail::weight>();
  buffer_.template at<detail::weight>(i).add_weight(w);
}

template <template <class> class Allocator>
inline typename adaptive_storage<Allocator>::value_type
adaptive_storage<Allocator>::value(std::size_t i) const {
  switch (buffer_.type_.id_) {
  case 0:
    break;
  case 1:
    return buffer_.template at<uint8_t>(i);
  case 2:
    return buffer_.template at<uint16_t>(i);
  case 3:
    return buffer_.template at<uint32_t>(i);
  case 4:
    return buffer_.template at<uint64_t>(i);
  case 5:
    return static_cast<value_type>(buffer_.template at<detail::mp_int>(i));
  case 6:
    return buffer_.template at<detail::weight>(i).w;
  }
  return 0.0;
}
...
adaptive_storage<Allocator>::variance(std::size_t i) const {
  ...
  case 5:
    return static_cast<value_type>(buffer_.template at<detail::mp_int>(i));
  case 6:
    return buffer_.template at<detail::weight>(i).w2;
  }
  return 0.0;
}
...
template <template <class> class OtherAllocator>
adaptive_storage<Allocator> &adaptive_storage<Allocator>::
operator+=(const adaptive_storage<OtherAllocator> &o) {
  ...
  case 0:
    return *this;
  case 1:
    buffer_.template convert<uint8_t>();
    break;
  case 2:
    buffer_.template convert<uint16_t>();
    break;
  case 3:
    buffer_.template convert<uint32_t>();
    break;
  case 4:
    buffer_.template convert<uint64_t>();
    break;
  case 5:
    buffer_.template convert<detail::mp_int>();
    break;
  case 6:
    buffer_.template convert<detail::weight>();
  ...
  for (std::size_t i = 0; i < buffer_.size_; ++i) {
    ...
    case 1:
      x = o.buffer_.template at<uint8_t>(i);
      break;
    case 2:
      x = o.buffer_.template at<uint16_t>(i);
      break;
    case 3:
      x = o.buffer_.template at<uint32_t>(i);
      break;
    case 4:
      x = o.buffer_.template at<uint64_t>(i);
      break;
    case 5:
      x = o.buffer_.template at<detail::mp_int>(i);
      break;
    ...
    // case 0 already covered above
    case 1:
      detail::add_impl<B, uint8_t, detail::mp_int>::apply(buffer_, i, x);
      break;
    case 2:
      detail::add_impl<B, uint16_t, detail::mp_int>::apply(buffer_, i, x);
      break;
    case 3:
      detail::add_impl<B, uint32_t, detail::mp_int>::apply(buffer_, i, x);
      break;
    case 4:
      detail::add_impl<B, uint64_t, detail::mp_int>::apply(buffer_, i, x);
      break;
    case 5:
      detail::add_impl<B, detail::mp_int, detail::mp_int>::apply(buffer_, i, x);
      break;
    case 6:
      detail::add_impl<B, detail::weight, detail::mp_int>::apply(buffer_, i, x);
      break;
    }
  }
  return *this;
...
template <typename OtherStorage, typename>
adaptive_storage<Allocator> &adaptive_storage<Allocator>::
operator+=(const OtherStorage &other) {
  using B = decltype(buffer_);
  using T = typename OtherStorage::value_type;
  using U = detail::to_storage_type<T>;
...
operator==(const adaptive_storage<OtherAllocator> &o) const {
  ...
  switch (buffer_.type_.id_) {
  case 0:
    break; // handled in return
  case 1:
    return detail::cmp_impl<TB, OB, uint8_t>::apply(buffer_, o.buffer_);
  case 2:
    return detail::cmp_impl<TB, OB, uint16_t>::apply(buffer_, o.buffer_);
  case 3:
    return detail::cmp_impl<TB, OB, uint32_t>::apply(buffer_, o.buffer_);
  case 4:
    return detail::cmp_impl<TB, OB, uint64_t>::apply(buffer_, o.buffer_);
  case 5:
    return detail::cmp_impl<TB, OB, detail::mp_int>::apply(buffer_, o.buffer_);
  case 6:
    return detail::cmp_impl<TB, OB, detail::weight>::apply(buffer_, o.buffer_);
  }
...
template <template <class> class Allocator, typename OtherStorage,
          typename = typename std::enable_if<
              !(detail::has_weight_support<OtherStorage>::value)>::type>
inline bool operator==(const OtherStorage &a,
                       const adaptive_storage<Allocator> &b) {
  return b == a;
}
